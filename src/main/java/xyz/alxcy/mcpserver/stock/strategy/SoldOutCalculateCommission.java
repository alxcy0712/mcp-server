 ...
 */
@Service
public class SoldOutCalculateCommission implements CalculateCommissionStrategy{
    ...
    public BigDecimal calculateCommission(BigDecimal stockPrice, int stockNumber) {
        ...
        // 3. 计算印花税
        BigDecimal stampDuty = totalAmount.multiply(SOLD_OUT_STAMP_DUTY);
        // 4. 计算其他费用
        BigDecimal otherFee = totalAmount.multiply(OTHER_FEE);
        // 5. 计算总佣金
        return commission.add(stampDuty).add(otherFee).setScale(SCALE, RoundingMode.HALF_UP);
    }
}
